...
num_topics = 600
num_words=50
print('主题数%d个，关键字%d个......' % (num_topics,num_words))
...
for n in range(len(doc_lda)):
    Topic=doc_lda[n]
    if len(Topic)==0:
        prams = (institution_paper_list[n][0],"top_其他", json.dumps({}, ensure_ascii=False),
                 json.dumps({}, ensure_ascii=False))
        sql = 'insert into lda3 values(%s,%s,%s,%s)'
    ...
    for key in DocWord[n]:
        if key in d.keys():
            t[key]=d[key]
    topic=c1[0][0]
    prams=(institution_paper_list[n][0],"top_"+str(topic),json.dumps(d,ensure_ascii=False),json.dumps(t,ensure_ascii=False))
    sql='insert into lda3 values(%s,%s,%s,%s)'
    list = dbs.exe_sql(sql, prams)